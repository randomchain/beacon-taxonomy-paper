...
A general example of autocratic collector beacon types can be seen in \Vref{fig:autocratic_beacon}, where the green marking depicts the components under total control of the beacon operator.
...
Scaling \gls{mpc} protocols can also be difficult, although \citetalias{cascudo2017scrape} does improve on the performance of such protocols, especially in relation to a beacon, where the outcome is required to be published at regular intervals.
...
\begin{figure}[hbt]\pgfdeclarelayer{background}
    ...
    \begin{tikzpicture}[auto]
        ...
        \draw[arrow] (computation.south) to [out=250, in=190, looseness=2] node (rprime) {$O'$} (computation.west);
        \begin{pgfonlayer}{background}
