...
An example of a specialized \gls{mpc} beacon protocol can be seen in \Cref{fig:mpc_beacon}, where the green marking covers the scope of the sequential \gls{mpc} between participants.
...
\begin{figure*}[hbt]\pgfdeclarelayer{background}
    ...
    \begin{tikzpicture}[auto]
        ...
        \draw[arrow] (computation) -- node {$O_t$} (users);
        \draw[arrow] (computation.east) to [out=-10, in=-90, looseness=2] node (rprime) {$O'$} (computation.south);
        \begin{pgfonlayer}{background}
...
In \Cref{fig:transparent_beacon} we show an example of a beacon of transparent authority, where the green markings indicate where the authority has power to affect the outcome.
