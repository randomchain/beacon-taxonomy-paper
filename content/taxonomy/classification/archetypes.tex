...
The most naïve implementation of a randomness beacon is an autocratic beacon, which collects input from an internal source. Examples of this type include \citetalias{nistbeacon} and \citetalias{fischer2011publicrandomnessservice}.
...
There is also no bottleneck in collecting entropy for the input, since it only relies on measuring something in the real world e.g.\ background radiation. E.g. \citetalias{nistbeacon} provides fresh randomness every 60 seconds\cite{nistbeacon}.
...
The availability can also be severely hindered if one or multiple adversaries decide to drop out mid-execution. It can also be dependent on having a honest majority of users \cite{cascudo2017scrape}, which can severely impact availability in a public setting.
...
Examples of this type of beacon are \citetalias{clark2010use}, \citetalias{baigneres2015trap}, \citetalias{bonneau2015bitcoin}, \citetalias{bentov2016bitcoin}, \citetalias{bunz2017proofsof}, \citetalias{lenstra2015random} and \citetalias{randao} \stefan{give citations}, making it the most common archetype.
...
Similarly \citetalias{randao}, a smart contract on the Ethereum blockchain, has an incentive structure to make users provide input in the hopes of earning profits from making randomness available to other smart contracts.
