...
Rabin is only concerned with emitting integers in some (small) range because there, for his use cases, is a trade-off between security and time, such that a larger range provides better security but longer transaction times. We will however not impose a limit or give suggestions for the range. This ultimately depends on the use case of the random number. We will therefore introduce a property called \emph{Entropy Guarantee}, meaning that it must be possible to guarantee a level of entropy. As an example, a lottery picking one human on Earth requires at least $\log_2(7.5~\text{billion}) \approx 33~\text{bits}$ of entropy.

\mtjnote{Write something about availability}

As stated, Rabin's definition requires trust in a third party, with no way to ensure the honesty of that party and closely matches the \gls{nist} randomness beacon.
To avoid having to trust blindly in a third party, we will look for an alternative definition that also considers trust. \mtjnote{And this leads to verifiability.}

\mtjnote{Pls stahp reading here.}
