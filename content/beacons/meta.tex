...
While we agree with these properties, we find especially the \emph{random} property to be vague and poorly motivated. Therefore, we will specify it in greater detail.

Unpredictability is the main property that governs the security of randomness beacon use cases. No party should be able to \enquote{predict} the output of the randomness beacon with higher probability than other parties. Because it is unpredictable it must also necessarily be random. If it was not random, some

We will relax the requirement of random, to just being picking uniform distribution
