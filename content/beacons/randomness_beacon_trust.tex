...
\begin{description}
    ...
    \item[Verifiability]
        ...
        The input to the beacon must be available for users to verify a given output, unless the beacon is able to produce a proof, which allows users to verify. \msmnote{this could be interesting to expand upon, to allow for anonymized beacon interaction} \stefan{yes please elaborate. how can i verify a computation has been done corectly?}.
...
For randomness beacons there exist two main venues of attack: \emph{availability} and \emph{integrity} \stefan{what does that include? mention here whether you mean that the number is wrong, the computation is wrong or that different users see different numbers? or the input is tempered with.}.
...
\begin{description}
    ...
    \item[Denial of Input] For beacons that use user input, the attacker can overwhelm the operator with input \stefan{well why not just block the inputs of others directly? (Not sure what he wants here)} \stefan{instead of overloading the beacon the attacker could also cut the channels so that some users cannot reach the beacon operator anymore, or introduce delays, right?}, preventing other users from contributing, and potentially biasing the input, violating the unpredictability and randomness properties of the beacon.
